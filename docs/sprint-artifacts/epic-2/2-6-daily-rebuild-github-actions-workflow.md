...
- [x] [Review][Defer→Routed to 2.9 2026-05-10] **Issue spam / no deduplication** — persistent failures create one GitHub Issue per scheduled run (daily for `daily-rebuild.yml`, weekly for the monitor); no search-first-or-reopen logic. [`.github/workflows/daily-rebuild.yml`, `.github/workflows/third-party-asset-monitor.yml`] — routed to Story 2.9 (Epic 2 Hardening) AC #4.
- [x] [Review][Defer→Routed to 2.9 2026-05-10] **Failure issue body omits failing step name** — `context.sha` + run URL require log drill-down to identify the specific failing step; body says "check the run logs" but doesn't short-circuit to the step. [`.github/workflows/daily-rebuild.yml`, Notify step body] — routed to Story 2.9 (Epic 2 Hardening) AC #5.
- [x] [Review][Defer→Routed to 2.9 2026-05-10] **`context.sha` misleading for cron runs** — SHA in the notify body is HEAD of main at schedule time, not the checked-out tag commit (cron builds use tag content). Confusing if the failure is in the Hugo build against tag content. [`.github/workflows/daily-rebuild.yml`, line ~220] — routed to Story 2.9 (Epic 2 Hardening) AC #5.
- [x] [Review][Defer→Routed to 2.9 2026-05-10] **Markdown injection in issue bodies** — `FAILED_LIST` and failure messages are embedded verbatim; URLs with Markdown control chars render as formatting (cosmetic, no security impact). [`.github/workflows/third-party-asset-monitor.yml`, Notify on drift script] — routed to Story 2.9 (Epic 2 Hardening) AC #6.
- [x] [Review][Defer→Routed to 2.9 2026-05-10] **XSS via `javascript:` URIs in webmention links** — `<a href="{{ .author_url }}">` and `<a href="{{ .url }}">` pass `javascript:` URIs unguarded; Hugo auto-escaping blocks HTML injection but not protocol injection. Pre-existing, not introduced by this diff. [`layouts/_partials/widgets/webmention-group.html`] — routed to Story 2.9 (Epic 2 Hardening) AC #1 (security-grade item, ship first).
