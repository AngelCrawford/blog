...
- [x] [Review][Defer→Routed to 2.9 (build-smoke) + Story 3.2 (Playwright) 2026-05-10] AC #8 XSS smoke test validates only clean fixture data — fixture has no HTML payloads, so the test cannot verify Hugo's auto-escape is active; add an adversarial fixture entry (e.g., `content: "<script>alert(1)</script>"`) to make the assertion meaningful. [`tests/build/build-smoke.test.mjs:945`] — build-smoke adversarial fixture routed to Story 2.9 (Epic 2 Hardening) AC #7; live-render Playwright path remains routed to Story 3.2 when real webmention data lands.
