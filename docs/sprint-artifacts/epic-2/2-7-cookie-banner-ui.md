...
5. **Banner styled non-intrusive (bottom-fixed, semi-transparent, matches site theme).** New SCSS module at `assets/scss/elements/cookie-banner.scss` (matches `assets/scss/elements/{badge,box,button,pagination,ribbon,search,tooltip}.scss` ITCSS-inspired convention per `architecture.md` lines 233–241). Wired into `assets/scss/main.scss` via a new `@use "elements/cookie-banner";` line in the "Elements" block (after `@use "elements/tooltip";`). Style baseline:
   - `position: fixed; bottom: 0; left: 0; right: 0; z-index: 100;` — site uses `z-index: 999` for `#back-to-top` (main.scss line 199) so 100 stays below it (back-to-top must remain clickable when banner is visible).
...
9. **No automated tests added in this story** (testability guard, scope-limit). Test infrastructure (`tests/build/`, `tests/e2e/`) is bootstrapped by Story 1.1 (status: ready-for-dev, not landed at this story's drafting time per `sprint-status.yaml`). If Story 1.1 has merged before this story implements, an OPTIONAL Playwright test (`tests/e2e/cookie-banner.spec.ts` — ~30 lines) verifying (a) banner appears on first visit, (b) banner stays dismissed within session after close, (c) banner does not appear on `/pages/datenschutz/` may be added — but only if the test infrastructure is in place. Skip if test infra absent. Manual QA per the Tasks section is sufficient for the 0.5-day estimate.
...
[Source: layouts/baseof.html (lines 1–38) — partial inclusion site for the new cookie-banner partial]
...
[Source: config/_default/params.yaml (lines 17–29) — CSP scriptsrc/stylesrc allow self-hosted bundle; no CSP changes needed for this story]
...
- [ ] **Audit current state of `assets/js/gdpr.js`, `layouts/baseof.html`, and `layouts/_partials/_base/head.html`** (AC: 1, 4) [Source: assets/js/gdpr.js, layouts/baseof.html, layouts/_partials/_base/head.html]
  ...
  - [ ] Confirm `baseof.html` still ends with the back-to-top button + footer-bundle script tag (lines 15–34 of current file). The new partial inclusion goes between the `</footer>` close and the footer-bundle script (at ~line 25, before `<!-- Footer Scripts -->`).
...
- [ ] **Wire the partial into `layouts/baseof.html`** (AC: 1, 7) [Source: layouts/baseof.html lines 1–38, AC #7 noindex gate]
  - [ ] Add the partial inclusion as the last child of `<body>`, **before** the `<!-- Footer Scripts -->` block. Suggested placement at ~line 25 of current `baseof.html`:
    ```go-html-template
...
- [ ] **Create the SCSS module `assets/scss/elements/cookie-banner.scss`** (AC: 5, 7) [Source: AC #5 styling baseline, AC #7 print-media rule, assets/scss/main.scss lines 38–44 (Elements block), assets/scss/elements/{badge,box,button}.scss (sibling pattern), assets/scss/vars/_colors.scss (variable usage)]
  - [ ] Create new file `assets/scss/elements/cookie-banner.scss`. Suggested baseline (refine theme variables for actual project tokens at implementation time — read `assets/scss/vars/_colors.scss` and `_helpers.scss` first):
    ...
    ```
  - [ ] **Read `assets/scss/vars/_helpers.scss` and `_colors.scss` first** to use the actual variable names/imports the project uses (`$gold`, `$gold-light`, `$dark`, `$text` may or may not be the exact tokens). Adjust the SCSS to compile against the real variables before committing.
  - [ ] Wire into `assets/scss/main.scss` by adding `@use "elements/cookie-banner";` to the Elements block (after `@use "elements/tooltip";` at line 44 of current `main.scss`).
...
The banner sits at `z-index: 100` to remain below the back-to-top button (`z-index: 999`, see `assets/scss/main.scss` line 199). Site-wide z-index conventions are loose — there is no `_z-index.scss` stack documentation; this story's choice of `100` for "above content, below page-chrome" sets a reusable precedent.
...
- **CSP impact: none.** The new JS lives in `assets/js/gdpr.js` which is concatenated into `js/bundle.js` and served from `'self'` — already allowed by `params.yaml` line 25 `scriptsrc: ["'self'", "https://cloud.umami.is"]`. No inline scripts, no inline event handlers, no new external hosts.
- **Test infrastructure status:** Story 1.1 bootstraps `tests/build/` and `tests/e2e/` (`ready-for-dev`, not landed at this story's drafting time per `sprint-status.yaml`). This story does NOT have automated tests — manual QA per the Tasks section is sufficient for the 0.5-day estimate.
...
[Source: config/_default/params.yaml (lines 17–29) — CSP allowlist (no changes needed)]
...
**From Story 2.6 (Daily Rebuild GitHub Actions Workflow) — Status: drafted (not yet implemented).**

The create-story workflow rule treats anything below `in-progress`/`review`/`done` as `"Previous story not yet implemented"`. Story 2.6 is `drafted`, so no implementation learnings exist to forward. **However, sibling-draft patterns from Stories 2.1–2.6 are directly load-bearing** for this story (same convention layer Story 2.6 documented in its own Learnings section):

- **Verbatim-from-epics + testability-guards pattern (Stories 2.1–2.6 drafts):** ACs 1–7 are sourced verbatim from epics.md; ACs 8–9 are workflow-added regression/scope-limit guards labelled at the top of the AC Source & Reconciliation block. Same convention applied here.
- **Architecture-vs-implementation reconciliation pattern (Stories 2.1–2.6 drafts):** when an AC's wording disagrees with the project state or with a Critical Agent Rule, document the divergence in a "Convention reconciliation" sub-block, follow the rule/state, and note any architecture-doc cleanup as a separate todo. Applied here for: (a) AC #3 privacy URL (`/pages/privacy/` epics text vs `/pages/datenschutz/` actual deployed slug — same reconciliation Story 2.5 made); (b) AC #4 "existing gdpr.js" (jQuery legacy vs Critical Agent Rule #5 vanilla-JS mandate); (c) AC #6 `role="dialog"` (literal AC vs ARIA-precise alternatives — followed AC verbatim with `aria-modal="false"` to satisfy both).
...
[Source: docs/sprint-artifacts/sprint-status.yaml — current development_status (epic-1: 1-1 → 1-5 ready-for-dev; epic-2: 2-1 → 2-5 ready-for-dev, 2-6 drafted, 2-7 was the next backlog story, now drafted)]
...
- [Source: layouts/baseof.html (lines 1–38)] — partial inclusion site
...
- [Source: assets/scss/main.scss (lines 38–44)] — Elements block for `@use "elements/cookie-banner";` wiring
...
- [Source: config/_default/params.yaml (lines 17–29)] — CSP allowlist (no changes needed; bundle is `'self'`-served)
